...
export const Input: React.FC<InputProps> = ({ label, id, validationStatus = 'idle', ...props }) => (
...
      />
      <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
        {validationStatus === 'loading' && <SpinnerIcon />}
        {validationStatus === 'success' && <CheckCircleIcon />}
        {validationStatus === 'error' && <XCircleIcon />}
      </div>
    </div>
  </div>
);
...
// ... existing DownloadIcon, TrashIcon components ...
...
export const ToggleSwitch: React.FC<{ label: string; enabled: boolean; onChange: (enabled: boolean) => void; }> = ({ label, enabled, onChange }) => (
    <div className="flex items-center justify-between">
        ...
        <button
            type="button"
            className={`${enabled ? 'bg-brand-600' : 'bg-slate-700'} relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:ring-offset-slate-900`}
