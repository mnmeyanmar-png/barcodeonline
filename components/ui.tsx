...
export const Input: React.FC<InputProps> = ({ label, id, validationStatus = 'idle', ...props }) => {
    const statusIcon = () => {
        switch (validationStatus) {
            case 'loading':
                return <SpinnerIcon className="h-5 w-5 text-slate-400" />;
            case 'success':
...
export const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ label, enabled, onChange }) => (
    <div className="flex items-center justify-between">
        ...
        <button
            ...
            } relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:ring-offset-slate-900`}
        >
            <span
                className={`${
...
// --- Icon Components ---
export const SpinnerIcon: React.FC<{className?: string}> = ({className = "-ml-1 mr-3 h-5 w-5 text-white"}) => (
